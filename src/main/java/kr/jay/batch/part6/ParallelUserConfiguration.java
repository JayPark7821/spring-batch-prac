...
public class ParallelUserConfiguration {
	...
	public ParallelUserConfiguration(final JobBuilderFactory jobBuilderFactory,
	...
	public Flow saveUserFlow(){
		final TaskletStep saveUserStep = this.stepBuilderFactory.get(JOB_NAME + "_saveUserStep")
	...
	public Flow splitFlow(@Value("#{jobParameters[date]}") String date) throws Exception {
		final SimpleFlow userLevelUpFlow = new FlowBuilder<SimpleFlow>(JOB_NAME + "_userLevelUpFlow")
	...
	private Flow orderStatisticsFlow(String date) throws Exception {
		return new FlowBuilder<SimpleFlow>(JOB_NAME + "_orderStatisticsFlow")
			.start(new JobParametersDecider("date"))
